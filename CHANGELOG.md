...
- **S1.10 — first Fortran-vs-JAX regression**: `tests/regression/test_fortran_golden.py` compares the JAX simulator against a committed Fortran golden produced by `box.exe` from the SOM-TOMAS reference, using the goldens loader and compare primitives from `atmos-jax-common`. The golden is the same `tests/fixtures/sample_run/` shipped by `atmos-jax-common`. Reference is `_saprcgc.dat` (ppm, post-chemistry) — `_gc.dat` is in kg/bag and reflects post-condensation state, which is out of scope for `som-jax` (gas-phase only per D2). Tolerances: 15% L2 / final-rel for the well-resolved 33+ species above the 1e-10 ppm magnitude floor; 10% for the four BL20 first-gen products specifically; below-floor species checked for "stays low". `atmos-jax-common` added as a dev dep (git URL, since it's not on PyPI yet). Total: **67 tests pass** (4 new regression tests + 63 pre-existing).
- Figure: `docs/figures/s1.10/regression_overview.png` — top: per-species relative L2 bar chart with regression and master-plan tolerance lines; bottom: candidate-vs-reference final-time scatter (log-log) for all 40 SOM species with y=x diagonal and ±15% bands.
- Master-plan annotation: the systematic ~7-9% JAX-vs-Fortran gap on cascade products is attributable to Fortran `INTEGR2`'s `rtol=2e-3` (per `box.f:828`) — coarser than JAX's `rtol=1e-10`. JAX produces consistently slightly higher values for cascade products, which is read as Fortran losing some mass to its own integrator error; the GENVOC analytic-decay test exactly matches both, so the chemistry is correct at the source.
