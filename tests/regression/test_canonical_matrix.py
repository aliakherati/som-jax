...
fixture) to the canonical-run matrix shipped by ``atmos-jax-common``
(8 in-scope runs sweeping endtime 10 min → 24 h, OH dose ~10×, and
GENVOC magnitude ~100×).

What we test, and what we don't
-------------------------------
For each in-scope run we compare the JAX simulator against Fortran
...
Two runs (``cold``, ``hot``) are skipped: the JAX network's
``k_OH`` is parsed as the .doc-listed value at 298 K with no
Arrhenius temperature dependence. Fortran computes ``k(T)`` at
simulation time, so at 273 / 323 K the rates differ by ~7-9%
(BL20 has ``B = -1`` in its Arrhenius parametrisation). T-dependent
rates is its own follow-up chunk.
"""
...
# Tolerance budget for the tier-1 comparison. Even on the 24 h
# ``very_long`` run the worst-case GENVOC error is sub-percent; 1%
# is generous and survives the float32 noise that compounds in the
# deeper cascade (which we don't test here anyway).
_TIER1_L2_TOL = 0.01
...
def _t_dependent_rates_skip_reason(run_id: str) -> str:
    return (
        f"{run_id}: JAX network's k_OH is parsed at 298 K with no Arrhenius "
        f"T dependence; Fortran's rates at this temperature differ by ~7-9% "
        f"(BL20 has B = -1 in its parametrisation). T-dependent rates is a "
        f"follow-up chunk."
    )


def _matrix_run_params() -> list[pytest.param]:
    """Build the parametrize() table from the manifest, marking
    out-of-scope runs as skip."""
    matrix = load_manifest(default_manifest_path())
    params: list[pytest.param] = []
    for run in matrix.runs:
        if run.run_id in ("cold", "hot"):
            params.append(
                pytest.param(
                    run.run_id,
                    marks=pytest.mark.skip(reason=_t_dependent_rates_skip_reason(run.run_id)),
                    id=run.run_id,
                )
            )
        else:
            params.append(pytest.param(run.run_id, id=run.run_id))
    return params
...
def _run_jax_tier1(run: CanonicalRun, golden: GoldenRun, network: SOMNetwork):
    ...
    ``(jax_block, fortran_block, names)`` arrays for tier-1 species
    only."""
    oh_ppm = float(
        molec_cm3_to_ppm(
...
def test_in_scope_runs_cover_each_axis() -> None:
    ...
    axis of the matrix (besides T, which is skipped). Catches a
    regression where someone disables too many runs."""
    matrix = load_manifest(default_manifest_path())
    in_scope = {r.run_id for r in matrix.runs} - {"cold", "hot"}
    times = {r.params.endtime_h for r in matrix.runs if r.run_id in in_scope}
    assert len(times) >= 4, f"only {len(times)} distinct endtimes covered"
    ohs = {r.params.OH_molec_per_cm3 for r in matrix.runs if r.run_id in in_scope}
    assert len(ohs) >= 3, f"only {len(ohs)} distinct OH levels covered"
    vocs = {r.params.ippmprec_ppm for r in matrix.runs if r.run_id in in_scope}
